...
class min_MITOTrainer(Trainer):
    ...
    def compute_ref_log_probs(self, batch: dict[str, torch.LongTensor]) -> dict:
        ...
        return (
            ...
            ref_model_output["chosen_logits"],
            ref_model_output["rejected_logits"],
            ref_model_output["chosen_labels"],
            ref_model_output["rejected_labels"],
            )
    ...
    def dpo_loss(
        ...
        # print(f'self.reference_free: {self.reference_free}')
        # print(f'chosen_logps.shape: {chosen_logps.shape}, rejected_logps.shape:{rejected_logps.shape}, ref_chosen_logps.shape: {ref_chosen_logps.shape}, ref_rejected_logps.shape:{ref_rejected_logps.shape}')
    ...
    def get_batch_loss_metrics(
        ...
        batch.keys()
        (['prompt_input_ids', 'prompt_attention_mask', 'chosen_input_ids', 
        'chosen_attention_mask', 'rejected_input_ids', 'rejected_attention_mask'])
        model_output.keys():
        (['chosen_logps', 'rejected_logps', 'chosen_labels', 'rejected_labels',
          'chosen_logits', 'rejected_logits', 'mean_chosen_logits', 'mean_rejected_logits'])
        """
        metrics = {}
        ...
        if "ref_chosen_logps" in batch and "ref_rejected_logps" in batch:
            ...
            ref_chosen_logps, ref_rejected_logps, ref_chosen_logits, ref_rejected_logits, ref_chosen_labels, ref_rejected_labels = self.compute_ref_log_probs(batch)
        ...
        ce_loss = self.ce_loss(
            ...
            model_output["chosen_labels"].reshape(-1)
        ).mean()
        ...
        if self.args.rpo_alpha is not None:
            total_loss = total_loss + self.args.rpo_alpha * model_output["nll_loss"]  # RPO loss from V3 of the paper
